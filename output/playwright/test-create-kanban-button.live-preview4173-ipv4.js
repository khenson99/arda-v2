...
(async () => {
  ...
  try {
    ...
    const sidePanel = page.locator('div.fixed.right-0.top-0.z-50');
    await sidePanel.waitFor({ timeout: 30000 });
    const panelInputs = sidePanel.locator('input');
    await panelInputs.nth(0).fill(itemCode);
    await panelInputs.nth(1).fill(`Playwright Item ${itemCode}`);
    await panelInputs.nth(2).fill('Playwright Supplier');
